...
public class concolictestingv2 extends ConcolicTestGeneration {
    ...
    private static TestResult startGenerating(int id, TestGeneration.Coverage coverage) throws InvocationTargetException, IllegalAccessException, ClassNotFoundException, NoSuchFieldException, IOException, InterruptedException {
        ...
        for (int i =  0; i < evaluatedValues.length; i++) {
            System.out.println(evaluatedValues[i].getClass().getName());
        }

        TestGeneration.writeDataToFile("", FilePath.concreteExecuteResultPath, false);

        System.out.println(FilePath.concreteExecuteResultPath);
        ...
        System.out.println("MarkedStatements: " + markedStatements.size());
        ...
        double score = calculateFullTestSuiteCoverage(coverage);
        testResult.setFullCoverage(score);
    ...
    private static void setup(String path, String className, String methodName, TestGeneration.Coverage coverage) throws IOException, InterruptedException {
        System.out.println("Setting up coverage for: " + className + "." + methodName);
...
//        relative = relative.replace("\\", "/");
    ...
    private static void setupCfgTree(TestGeneration.Coverage coverage) {
        ...
        if (TestGeneration.cfgBeginNode == null) {
            System.out.println("vaicalon");
        }
        ...
        CfgNode f = ASTHelper.generateCFG(block, TestGeneration.compilationUnit, firstLine, TestGeneration.getCoverageType(coverage));
        Set<CfgNode> visited1 = new HashSet<>();
    }
