...
import com.opensymphony.xwork2.config.providers.*;
 ...
 */
public class JciaXmlConfigurationProvider extends XmlConfigurationProvider {


    private static final Logger LOG = LogManager.getLogger(XmlConfigurationProvider.class);
    private static final Logger logger = LogManager.getLogger(JciaXmlConfigurationProvider.class);

    private List<Document> documents;
    private Set<String> includedFileNames;
    private String configFileName;
    private JciaObjectFactory objectFactory;
    ...
    public JciaXmlConfigurationProvider(String classPath) {
        this("xwork.xml", true, classPath);
    }

    public JciaXmlConfigurationProvider(String filename, String classPath) {
        this(filename, true, classPath);
    }

    public JciaXmlConfigurationProvider(String filename, boolean errorIfMissing, String classPath) {
        this.configFileName = filename;
        this.errorIfMissing = errorIfMissing;
        this.basePath = classPath;
    ...
    public void setObjectFactory(ObjectFactory objectFactory) {
        JciaObjectFactory jciaObjectFactory = new JciaObjectFactory(objectFactory);
        this.objectFactory = jciaObjectFactory;
    }
    ...
    public boolean equals(Object o) {
        ...
        final JciaXmlConfigurationProvider xmlConfigurationProvider = (JciaXmlConfigurationProvider) o;
    ...
    protected void addAction(Element actionElement, PackageConfig.Builder packageContext) throws ConfigurationException {
        ...
        // className should be org.apache.struts2.dispatcher.DefaultActionSupport if it's not set
        String className = actionElement.getAttribute("class");
        if (className == null || className.trim().isEmpty()) {
            className = "org.apache.struts2.dispatcher.DefaultActionSupport";
        }

        //methodName should be execute() if it's not set
...
//                clazz.getConstructor(new Class[]{});
    ...
    protected void addResultTypes(PackageConfig.Builder packageContext, Element element) {
        ...
        for (int i = 0; i < resultTypeList.getLength(); i++) {
            ...
            if (clazz != null) {
                ...
                switch (className) {
                    // 11 default ResultTypes of Struts 2
                    case "com.opensymphony.xwork2.ActionChainResult":
                        paramName = "actionName";
                        break;
                    case "org.apache.struts2.result.ServletActionRedirectResult":
                        paramName = "actionName";
                        break;
                    case "org.apache.struts2.result.HttpHeaderResult":
                        paramName = ""; // indicate for DEFAULT_PARAM = null in Result Type Class;
                        break;
                    case "org.apache.struts2.result.StreamResult":
                        paramName = "inputName";
                        break;
                    case "org.apache.struts2.views.xslt.XSLTResult":
                        paramName = "stylesheetLocation";
                        break;
                    case "org.apache.struts2.result.ServletDispatcherResult":
                        paramName = "location";
                        break;
                    case "org.apache.struts2.views.freemarker.FreemarkerResult":
                        paramName = "location";
                        break;
                    case "org.apache.struts2.result.ServletRedirectResult":
                        paramName = "location";
                        break;
                    case "org.apache.struts2.result.VelocityResult":
                        paramName = "location";
                        break;
                    case "org.apache.struts2.result.PlainTextResult":
                        paramName = "location";
                        break;
                    case "org.apache.struts2.result.PostbackResult":
                        paramName = "location";
                        break;

                    // titles plugin result type
                    case "org.apache.struts2.views.tiles.TilesResult":
                        paramName = "location";
                        break;
                }
    ...
    protected List<InterceptorMapping> buildInterceptorList(Element element, PackageConfig.Builder context) throws ConfigurationException {
        ...
        for (int i = 0; i < interceptorRefList.getLength(); i++) {
            ...
            if (interceptorRefElement.getParentNode().equals(element) || interceptorRefElement.getParentNode().getNodeName().equals(element.getNodeName())) {
                ...
                interceptorList.add(new InterceptorMapping(interceptorRefElement.getAttribute("name"), null));
            }
        }
            ...
            if (resultElement.getParentNode().equals(element) || resultElement.getParentNode().getNodeName().equals(element.getNodeName())) {
                ...
                for (String name : resultNamesSet) {
                    ResultConfig resultConfig = new ResultConfig.Builder(name, resultClass)
                            .addParams(params)
                            .location(DomHelper.getLocationObject(resultElement))
                            .build();
    ...
    protected InterceptorStackConfig loadInterceptorStack(Element element, PackageConfig.Builder context) throws ConfigurationException {
        ...
        for (int j = 0; j < interceptorRefList.getLength(); j++) {
            ...
            InterceptorMapping mapping = new InterceptorMapping(interceptorRefElement.getAttribute("name"), null);
            config.addInterceptor(mapping);
        }
    ...
    /**
     * Get configuration files from specific path instead of project resources
     *
     * @param fileName
     * @return
     * @throws IOException
    ...
    protected Iterator<URL> getConfigurationUrls(String fileName) throws IOException {
        Iterator<URL> urls = LoaderUtil.getResources(basePath, fileName);
        return urls;
    }
